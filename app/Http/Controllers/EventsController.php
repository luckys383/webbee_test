...
class EventsController extends BaseController
{
    ...
    public function getFutureEventsWithWorkshops() {
        $workshopTableaName = (new Workshop)->getTable();
        $eventTableaName = (new Event)->getTable();
        $items = Event::join("$workshopTableaName as w", 'w.event_id', '=', "$eventTableaName.id")
                    ->with(['workshops'])
                    ->where("w.start", '>', Carbon::now())
                    ->select("$eventTableaName.*")
                    ->groupBy("$eventTableaName.id")
                    ->get();
        return $items;
